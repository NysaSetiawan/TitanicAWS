"""Train three candidate models on Space Titanic, compare, and package the winner.
...
Run from a SageMaker Notebook Instance:
    pip install xgboost scikit-learn pandas joblib mlflow
    python pipeline.py
"""
...
def main() -> None:
    ...
    print(f"Class distribution (train): "
          f"Not Transported (0)={int((y_train==0).sum())}, "
          f"Transported (1)={int((y_train==1).sum())}")

    pipelines = build_pipelines(df)
    results = {}
    ...
    print(f"\n  aws s3 cp {TARBALL_PATH} s3://your-bucket-name/titanic/model.tar.gz\n")
